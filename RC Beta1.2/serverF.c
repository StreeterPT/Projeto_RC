...
void receive_admin_command(char* command, int new_client, struct sockaddr_in addr, socklen_t addrlen, Shared_mem *shared_mem,int *logged_in);
...
void* udp(void* arg){
  ...
  while(1){
    ...
    receive_admin_command(buffer, udp_socket, client_addr, client_addr_len,shared_mem,&logged_in);
  }
  printf("Client disconnected\n");
  close(udp_socket);
  return NULL;
...
void receive_admin_command(char* command, int new_client, struct sockaddr_in addr, socklen_t addrlen, Shared_mem *shared_mem,int *logged_in){
    ...
    if (strcmp(command, "\n" ) != 0){
        ...
        if(strcmp("ADD_USER",space) == 0 && *logged_in == 1){
            ...
            add_user(username, password, role, new_client, addr, addrlen,shared_mem);
            return;
        }

        if(strcmp("DEL",space) == 0 && *logged_in == 1){
            ...
            remove_user(username, new_client, addr, addrlen,shared_mem);
            return;
        }

    
        if(strcmp("LIST",space) == 0 && *logged_in == 1){
            char *error_message = "<Invalid command>\n Correct Usage: LIST\n";
            // Check if the command has the correct number of arguments
            if(strtok(NULL, " ") != NULL){
                sendto(new_client, error_message, strlen(error_message), 0, (struct sockaddr*) &addr, addrlen);
        ...
        if(strcmp("QUIT_SERVER",space) == 0 && *logged_in == 1){
