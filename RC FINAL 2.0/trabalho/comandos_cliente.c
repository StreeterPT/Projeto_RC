#include "dados.h"
#define UDP_MULTICAST_PORT 5101
...
void send_content_to_class(char *class_name, char *content, int client_socket, Shared_mem *shared_mem) {
    //sem_wait(semShm);
    ...
    if(sendto(turma->udp_socket, message, strlen(message) + 1, 0,(struct sockaddr*)&turma->udp_address, sizeof(turma->udp_address)) < 0){ 
