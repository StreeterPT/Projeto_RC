...
bool check_type(char *name, char *type);
void* udp(void* udp_port);
void* tcp(void* tcp_port);
void process_client(int new_client);
void erro(char *msg);
void receive_client_command(char* command, int new_client);
void receive_admin_command(char* command, int new_client, struct sockaddr_in addr, socklen_t addrlen);
...
int main(int argc, char *argv[]){
  ...
  Shared_mem* shared_mem = create_shared_memory(shm_name);
  ...
  read_file(ficheiro_config,shared_mem);
...
void process_client(int new_client){
    ...
    while((bytes_received = read(new_client, buffer, BUF_SIZE - 1)) > 0){
        ...
        receive_client_command(buffer, new_client);
    }
    if(bytes_received == 0){
...
void* udp(void* arg){
  ...
  while(1){
    ...
    receive_admin_command(buffer, udp_socket, client_addr, client_addr_len);
  }
  printf("Client disconnected\n");
  close(udp_socket);
  return NULL;
...
void receive_client_command(char* command, int new_client){
    ...
    if(strcmp("LOGIN", space) == 0){
        ...
        if (check_user(username, password) && (check_type(username, "aluno") || check_type(username, "professor")) && !check_type(username, "admistrador")){
...
void receive_admin_command(char* command, int new_client, struct sockaddr_in addr, socklen_t addrlen){
    if (strcmp(command, "\n" ) != 0){
        ...
        if(strcmp("LOGIN",space) == 0){
            ...
            if (check_user(username, password) && check_type(username, "administrador")){
        ...
        if(strcmp("ADD_USER",space) == 0){
            ...
            add_user(username, password, role, new_client, addr, addrlen);
            return;
        }

        if(strcmp("DEL",space) == 0){
            ...
            remove_user(username, new_client, addr, addrlen);
            return;
        }

    
        if(strcmp("LIST",space) == 0){
            char *error_message = "<Invalid command>\n Correct Usage: LIST\n";
            // Check if the command has the correct number of arguments
            if(strtok(NULL, " ") != NULL){
                sendto(new_client, error_message, strlen(error_message), 0, (struct sockaddr*) &addr, addrlen);
            ...
            list_users(new_client, addr, addrlen);
            return;
        }

        if(strcmp("QUIT_SERVER",space) == 0){
...
bool check_type(char *name, char *type){
    ...
    bool flag = false;
    for (i = 0; i < n_users; i++){
        if (strcmp(user[i].name, name) == 0 && strcmp(user[i].type,type) == 0){
            flag = true;
            break;
        }
    }
