...
int client_logged_in = 0; // 0 - nao fez login 1- login como aluno 2- login como professor
int showed_menu=0;
int showed_menu2=0;
sem_t* semShm;
...
void receive_client_command(char* command, int new_client, Shared_mem *shared_mem){
    ...
    if(strcmp("LOGIN", space) == 0){
        ...
        if (check_user(username, password,shared_mem) && (check_type(username, "aluno",shared_mem) || check_type(username, "professor",shared_mem)) && !check_type(username, "admistrador",shared_mem)){
            if(check_type(username, "aluno",shared_mem)){client_logged_in=1;}
            if(check_type(username, "professor",shared_mem)){client_logged_in=2;}
            write(new_client,"OK\n",3);
    ...
    if(strcmp("LIST_CLASSES", space) == 0 && client_logged_in > 0){
        ...
        list_classes(new_client,shared_mem);
        return;
    }

    if(strcmp("LIST_SUBSCRIBED", space) == 0 && client_logged_in == 1){
        ...
        list_subscribed(logged_username,new_client,shared_mem);
        return;
    }

    if(strcmp("SUBSCRIBE_CLASS", space) == 0 && client_logged_in == 1){
        ...
        subscribe_user_to_class(logged_username,class_name, new_client,shared_mem);
        return;
    }
    if(strcmp("CREATE_CLASS", space) == 0 && client_logged_in==2){
        ...
        create_class(class_name, capacity, new_client,shared_mem);
        return;
    }

    if(strcmp("SEND", space) == 0 && client_logged_in==2){
        ...
        //send_content_to_class(char *class_name, char *content, int fd, struct sockaddr_in client_address, socklen_t client_address_len, Shared_mem *shared_mem);
        return;
    }
    write(new_client, "<Invalid command>\n", 18);
...
void read_file(char *filename,Shared_mem *shared_mem){
    ...
    while(fgets(buff, sizeof(buff), f)){
        ...
        shared_mem->users[shared_mem->n_users].n_subscribed_classes=0;
